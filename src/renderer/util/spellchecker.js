...
export class SpellChecker {
  ...
  // TODO(spell): Is LGPL code in app.asar allowed? As I understand, it's ok because the user can extract, replace and pack the app.asar.
  // TODO(spell): Enable automaticallyIdentifyLanguages - macOS only?
  // TODO(spell): Spell checking don't work on long lines or words with Electron >=v3.x which is the expected behavior :/ - see electron#16412
  //              --> We have to implement our own spellchecking system and highlight misspelled words or downgrade Electron.
  ...
  constructor (automaticallyIdentifyLanguages=false) {
    ...
    this.isHunspell = !isOSX || !!process.env['SPELLCHECKER_PREFER_HUNSPELL']
  ...
  async init (lang) {
    ...
    //              On macOS the user has to download dictionaries via system settings.
    if (!lang) {
      if (this.isHunspell) {
        // TODO(spell): Use "keyboard-layout" / "buildLikelyLocaleTable()" or "automaticallyIdentifyLanguages(true)"
        //              to determine the prefered language.
      }
    ...
    // Spellchecker is attached to the window's document.
...
// TODO(spell): Fork "electron-spellchecker" and add custom user dictories and apply the patch in "patch/".
//              Otherwise are word still red highlighted.
